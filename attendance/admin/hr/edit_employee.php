...
$stmt = $con->prepare("SELECT * FROM employees WHERE user_id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (empty($errors)) {
    if ($reset_device) {
      // Clear from employee_devices table
      $con->query("DELETE FROM employee_devices WHERE user_id = $id");
        ...
        <form method="POST" action="edit_employee.php?id=<?php echo $id; ?>" id="editEmployeeForm">
          <div class="row g-3">
            ...
            <div class="col-md-4">
              ...
              <input type="text" name="mobile" class="form-control" value="<?php echo htmlspecialchars($mobileVal); ?>">
            </div>

            <div class="col-md-4">
              <label class="form-label">Email</label>
              <input type="email" name="email" class="form-control" value="<?php echo htmlspecialchars($emailVal); ?>">
            </div>
              ...
              <div class="border rounded p-3" style="background-color: #f8f9fa;">
                ...
                foreach ($daysOfWeek as $day) {
                  ...
                  <div class="form-check form-check-inline">
                    <input class="form-check-input" type="checkbox" name="weekoff_days[]" value="<?php echo $day; ?>"
                      id="weekoff_<?php echo strtolower($day); ?>" <?php echo $checked; ?>>
            ...
            <!-- REGISTERED DEVICES + reset button -->
            <div class="col-md-6">
              <label class="form-label">Registered Devices</label>
              <?php
              $devRes = $con->query("SELECT COUNT(*) as c FROM employee_devices WHERE user_id = $id");
              $devCount = ($devRes && $dr = $devRes->fetch_assoc()) ? (int) $dr['c'] : 0;
              ?>
              <input type="text" class="form-control" value="<?php echo $devCount; ?> Device(s) Registered" disabled>
              <div class="form-text">
                Employee can register multiple devices up to the global limit.
              </div>

              <!-- Hidden field that JS will set to 1 when Reset pressed -->
              <input type="hidden" name="reset_device" id="resetDeviceInput" value="0">

              <button type="button" class="btn btn-sm btn-outline-danger mt-2" id="resetDeviceBtn">
                Reset All Devices
              </button>
            </div>
