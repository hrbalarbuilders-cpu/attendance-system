...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	if (!empty($selected)) {
		$stmt = $con->prepare("INSERT INTO leave_type_employees (leave_type_id, user_id) VALUES (?, ?)");
...
// Fetch all employees
$empRes = $con->query("SELECT user_id, emp_code, name, department_id FROM employees ORDER BY name ASC");
...
$assigned = [];
$mapRes = $con->query("SELECT user_id FROM leave_type_employees WHERE leave_type_id = " . $leaveId);
if ($mapRes) {
	while ($m = $mapRes->fetch_assoc()) {
		$assigned[] = (int) $m['user_id'];
	}
}
?>
<!DOCTYPE html>
		...
		<div class="page-wrapper w-100">
			...
			<div class="d-flex justify-content-between align-items-center mb-3">
				<div>
					...
					<small class="text-muted">Leave type: <?php echo htmlspecialchars($leaveType['name']); ?>
						(<?php echo htmlspecialchars($leaveType['code']); ?>)</small>
				</div>
								...
								<tbody>
									<?php if ($empRes && $empRes->num_rows > 0): ?>
										<?php while ($e = $empRes->fetch_assoc()):
											$isChecked = in_array((int) $e['user_id'], $assigned, true);
											$deptName = $deptMap[$e['department_id']] ?? '-';
											?>
											<tr>
												<td>
													<input type="checkbox" class="form-check-input emp-check"
														name="employee_ids[]" value="<?php echo (int) $e['user_id']; ?>" <?php echo $isChecked ? 'checked' : ''; ?>>
												</td>
