...
ensureColumnExists($con, 'geo_settings', 'geofence_polygon', '`geofence_polygon` LONGTEXT DEFAULT NULL');

ensureColumnExists($con, 'geo_settings', 'latitude', '`latitude` DECIMAL(10,8) DEFAULT NULL');
ensureColumnExists($con, 'geo_settings', 'longitude', '`longitude` DECIMAL(11,8) DEFAULT NULL');
ensureColumnExists($con, 'geo_settings', 'radius_meters', '`radius_meters` INT DEFAULT 150');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['location_form'])) {
  ...
  $geofence_polygon = trim($_POST['geofence_polygon'] ?? '');
  $latitude = isset($_POST['latitude']) ? (float) $_POST['latitude'] : null;
  $longitude = isset($_POST['longitude']) ? (float) $_POST['longitude'] : null;
  $radius_meters = isset($_POST['radius_meters']) ? (int) $_POST['radius_meters'] : 150;
  ...
  if ($geofence_polygon === '') {
    $errors[] = 'Polygon boundary is required. Use the paste tool.';
  }

  if (empty($errors)) {
    if ($id > 0) {
      $stmt = $con->prepare("UPDATE geo_settings SET location_group = ?, location_name = ?, geofence_polygon = ?, latitude = ?, longitude = ?, radius_meters = ? WHERE id = ?");
      $stmt->bind_param("sssdddi", $location_group, $location_name, $geofence_polygon, $latitude, $longitude, $radius_meters, $id);
      if ($stmt->execute()) {
      ...
      $stmt = $con->prepare("INSERT INTO geo_settings (location_group, location_name, geofence_polygon, latitude, longitude, radius_meters, is_active) VALUES (?, ?, ?, ?, ?, ?, 1)");
      $stmt->bind_param("sssddi", $location_group, $location_name, $geofence_polygon, $latitude, $longitude, $radius_meters);
      if ($stmt->execute()) {
        ...
        <table class="table table-hover table-borderless align-middle mb-0 w-100" style="font-size: 0.97rem;">
          <thead class="table-light" style="font-size:0.93em;">
            <tr>
              ...
              <th class="text-start px-2">Office Name</th>
          ...
          <tbody>
              ...
              while ($row = $list->fetch_assoc()):
                ...
                <tr>
                  ...
                  <td class="text-start">
                    ...
                    <div class="x-small text-muted">Polygon Boundary Set</div>
                  </td>
                  ...
                  <td class="text-center actions-cell">
                    <button type="button" class="btn btn-sm btn-outline-primary edit-location-btn"
                      ...
                      data-radius="<?php echo (int) ($row['radius_meters'] ?? 150); ?>"
                      data-polygon="<?php echo htmlspecialchars($row['geofence_polygon'] ?? ''); ?>">Edit</button>
                    <button type="button" class="btn btn-sm btn-outline-warning location-toggle"
                      data-id="<?php echo (int) $row['id']; ?>">Toggle</button>
                    <button type="button" class="btn btn-sm btn-outline-danger location-delete"
            ...
            <?php else: ?>
              <tr>
                <td colspan="5" class="text-muted text-center py-3">No locations defined yet.</td>
              </tr>
            <?php endif; ?>
          </tbody>
        </table>
...
<body>
  <div class="container py-4">
    <div class="d-flex justify-content-between align-items-center mb-3">
      ...
      <button type="button" class="btn btn-dark" data-bs-toggle="modal" data-bs-target="#locationModal">+ Add
        Point</button>
    </div>
              ...
              <?php if ($list && $list->num_rows > 0): ?>
                ...
                while ($row = $list->fetch_assoc()): ?>
                  <tr>
                    ...
                    <td class="small"><?php echo htmlspecialchars($row['latitude']); ?>,
                    ...
                    <td class="actions-cell">
                      <button type="button" class="btn btn-sm btn-outline-primary edit-location-btn"
                        ...
                        data-polygon="<?php echo htmlspecialchars($row['geofence_polygon'] ?? ''); ?>">Edit</button>
                      <a href="location_settings.php?toggle=<?php echo (int) $row['id']; ?>"
                        class="btn btn-sm btn-outline-warning">Toggle</a>
                      <a href="location_settings.php?delete=<?php echo (int) $row['id']; ?>"
                        class="btn btn-sm btn-outline-danger" onclick="return confirm('Delete this point?');">Delete</a>
                    </td>
                  </tr>
                <?php endwhile; ?>
              <?php else: ?>
                <tr>
                  <td colspan="7" class="text-muted">No locations defined yet.</td>
                </tr>
              <?php endif; ?>
            </tbody>
          </table>
