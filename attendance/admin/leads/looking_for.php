...
<body>
  ...
  <script>
    ...
    function loadLF(page = 1) {
      ...
      fetch(url).then(r => r.text()).then(html => {
        document.getElementById('lfList').innerHTML = html.replace(/<!--LF_TOTAL:\d+-->/,''); initLFHandlers(); attachLFPagination(); var foot = document.getElementById('lfPerPageFooter'); if (foot){ foot.value = currentLFPerPage; foot.onchange = function(){ currentLFPerPage = parseInt(this.value,10)||10; loadLF(1); }; } });
  }
  ...
  document.getElementById('btnAddLF').addEventListener('click', function () { document.getElementById('lookingForForm').reset(); document.getElementById('lookingForId').value = ''; document.getElementById('lfTypes').innerHTML = ''; addTypeGroup('', ''); if (lfModal) lfModal.show(); });
      ...
      document.getElementById('lookingForForm').addEventListener('submit', function (e) {
        e.preventDefault(); var id = document.getElementById('lookingForId').value; var url = id ? 'update_looking_for.php' : 'create_looking_for.php';
