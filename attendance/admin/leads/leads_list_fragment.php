...
$where_sql = '';
if (count($where_parts)) $where_sql = ' WHERE ' . implode(' AND ', $where_parts);
...
$hasLfType = false; $hasLfSub = false;
$c = $con->query("SHOW COLUMNS FROM leads LIKE 'looking_for_type_id'"); if ($c && $c->num_rows) $hasLfType = true;
$c = $con->query("SHOW COLUMNS FROM leads LIKE 'looking_for_subtypes'"); if ($c && $c->num_rows) $hasLfSub = true;
...
if ($totalPages > 1):
  ...
  if ($page > 1) echo '<li class="page-item"><a href="#" class="page-link leads-page-link" data-page="'.($page-1).'">Previous</a></li>';
  if ($start > 1) echo '<li class="page-item"><a href="#" class="page-link leads-page-link" data-page="1">1</a></li>' . ($start>2 ? '<li class="page-item disabled"><span class="page-link">...</span></li>':'' );
  for ($p = $start; $p <= $end; $p++){
    $cls = $p == $page ? ' page-item active' : ' page-item';
    echo '<li class="'.$cls.'"><a href="#" class="page-link leads-page-link" data-page="'.$p.'">'.$p.'</a></li>';
  }
  if ($end < $totalPages) echo ($end < $totalPages-1 ? '<li class="page-item disabled"><span class="page-link">...</span></li>':'') . '<li class="page-item"><a href="#" class="page-link leads-page-link" data-page="'.$totalPages.'">'.$totalPages.'</a></li>';
  if ($page < $totalPages) echo '<li class="page-item"><a href="#" class="page-link leads-page-link" data-page="'.($page+1).'">Next</a></li>';
  echo '</ul></nav>';
endif;
?>
<?php
