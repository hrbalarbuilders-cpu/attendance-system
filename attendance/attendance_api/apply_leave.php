...
$emp_id = isset($data['user_id']) ? (int) $data['user_id'] : (isset($data['employee_id']) ? (int) $data['employee_id'] : 0);
...
$conflictQuery = $con->prepare("SELECT id FROM leave_applications WHERE user_id = ? AND leave_type_id = ? AND status = 'pending' AND NOT (to_date < ? OR from_date > ?)");
...
$approvedConflict = $con->prepare("SELECT id, from_date, to_date FROM leave_applications WHERE user_id = ? AND status = 'approved' AND NOT (to_date < ? OR from_date > ?)");
...
try {
    $stmt = $con->prepare("INSERT INTO leave_applications (user_id, leave_type_id, from_date, to_date, reason, status, created_at) VALUES (?, ?, ?, ?, ?, 'pending', NOW())");
