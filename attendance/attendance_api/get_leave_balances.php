<?php
// Checked for user_id updates - none needed as it relies on leave_type_employees FK
...
try {
    ...
    $q = "SELECT lt.id, lt.code, lt.name, lt.yearly_quota FROM leave_types lt
          ...
          WHERE lte.user_id = $emp_id AND lt.is_active = 1";
    $res = $con->query($q);
    $balances = [];
    ...
    while ($row = $res->fetch_assoc()) {
        ...
        $usedSql = "SELECT COALESCE(SUM(DATEDIFF(LEAST(to_date, '$yearEnd'), GREATEST(from_date, '$yearStart')) + 1),0) AS used_days
                    ...
                    WHERE user_id = $emp_id AND leave_type_id = $ltid AND status = 'approved' AND NOT (to_date < '$yearStart' OR from_date > '$yearEnd')";
