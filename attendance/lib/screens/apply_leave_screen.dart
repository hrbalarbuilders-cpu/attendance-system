...
class _ApplyLeaveScreenState extends State<ApplyLeaveScreen> {
  ...
  Future<void> fetchLeaveTypes() async {
    ...
    if (widget.employeeId <= 0) {
      setState(() { errorMsg = 'Invalid employee id'; isLoading = false; });
      return;
    }
    try {
      final types = await leaveService.fetchLeaveTypes(widget.employeeId);
      final parsedTypes = types.map<Map<String, dynamic>>((e) => Map<String, dynamic>.from(e)).toList();
      // Also fetch balances for these leave types
      List<Map<String, dynamic>> balances = [];
      try {
        final balRes = await leaveService.fetchLeaveBalances(widget.employeeId);
        balances = balRes.map<Map<String, dynamic>>((b) => Map<String, dynamic>.from(b)).toList();
      } catch (_) {
      ...
      setState(() {
        ...
        if (balances.isNotEmpty) {
          ...
          if (leaveBalances.isEmpty) {
            leaveBalances = parsedTypes.map((t) => {'type': t['name'] ?? 'Unknown', 'total': 0, 'available': 0, 'used': 0}).toList();
          }
        }
        isLoading = false;
        debugInfo = null;
                    ...
                    child: Column(
                      ...
                      children: [
                        Padding(
                          ...
                          child: Text('Employee ID: ${widget.employeeId}', style: const TextStyle(fontWeight: FontWeight.w600)),
                        ),
                        ...
                        Row(
                          mainAxisSize: MainAxisSize.min,
                          children: [
                            ElevatedButton(onPressed: fetchLeaveTypes, child: const Text('Retry')),
                            const SizedBox(width: 8),
                            if (debugInfo != null)
                              ElevatedButton(
                                onPressed: () {
                                  Clipboard.setData(ClipboardData(text: debugInfo ?? ''));
                                  ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Copied debug info')));
                                },
                                  ...
                                  child: Builder(builder: (ctx) {
                                    ...
                                    Future<void> showLeaveMenu() async {
                                      ...
                                      final RelativeRect position = RelativeRect.fromLTRB(
                                          pos.dx,
                                          pos.dy + size.height,
                                          screenSize.width - (pos.dx + size.width),
                                          screenSize.height - (pos.dy + size.height));

                                      final selection = await showMenu<String?>(
                                        context: ctx,
                                        position: position,
                                        items: leaveTypes.map<PopupMenuEntry<String>>((type) {
                                          final name = type['name'] as String? ?? '';
                                          final balEntry = leaveBalances.firstWhere(
                                              (b) => (b['type'] as String? ?? '') == name,
                                              orElse: () => {'available': 0});
                                              ...
                                              child: Row(
                                                ...
                                                children: [
                                                  ...
                                                  Container(
                                                    ...
                                                    child: Text('Available: ${bal.toString()}', style: const TextStyle(fontSize: 13, color: Colors.grey)),
                                                  ),
                                                ],
                                              ),
                                            ),
                                      ...
                                      if (selection != null) {
                                        setState(() { selectedLeaveType = selection; });
                                      }
                                    }
                                    ...
                                    return Container(
                                      ...
                                      child: ListTile(
                                        ...
                                        title: Text(selectedLeaveType ?? 'Select leave type', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600)),
                                        trailing: Icon(Icons.arrow_drop_down_circle, color: Theme.of(context).primaryColor),
                                        onTap: showLeaveMenu,
                                      ),
                                    );
                                  }),
                                    ...
                                    onPressed: () async {
                                      ...
                                      try {
                                        ...
                                        messenger.showSnackBar(
                                          SnackBar(content: Text(ok ? (msg.isNotEmpty ? msg : 'Leave applied successfully') : (msg.isNotEmpty ? msg : 'Failed to apply leave'))),
                                        );
                                      } catch (e) {
                                        messenger.showSnackBar(
